...
#cfi = .987, RMSEA = .034, SRMR = .035
```

## Comparisons

```{r Comparisons}
comp_fit <- compareFit(RICLPM_mod1.fit, RICLPM_mod2.fit, RICLPM_mod2_alt.fit) # improvement although constrained lagged regression coefficients?
...
'
RICLPM1_mod1_ext.fit <- cfa(RICLPM_mod1_ext, 
  data = dat_SDQ, 
  missing = 'ML'
)
...
```{r mod2 version 1}
RICLPM_mod2_ext.fit <- cfa(RICLPM_mod1_ext,
  data = dat_SDQ, 
  missing = "ML", 
  group = "ASD_final",
  meanstructure = T, 
...
RICLPM_mod2_ext <- '
  ...
  RI_Conduct + RI_Hyper + RI_Emotional + RI_Peer ~~ 0*Int2_F + 0*Ext2_F
'
RICLPM1_mod2_ext_alt.fit <- cfa(RICLPM_mod2_ext, 
  data = dat_SDQ, 
  missing = 'ML',
  grou = "ASD_final",
  meanstructure = T, 
...
```{r Comparisons}
comp_fit <- compareFit(RICLPM1_mod1_ext.fit, RICLPM_mod2_ext.fit, RICLPM1_mod2_ext_alt.fit) # improvement although constrained lagged regression coefficients?
summary(comp_fit,  fit.measures = c("cfi", "rmsea", "srmr"))
```
...
```{r mod3 fitting}
RICLPM_mod3.fit <- lavaan(RICLPM_mod1_ext,
  data = dat_SDQ_ASD, 
  missing = "ML"
)
summary(RICLPM_mod3.fit, standardized = T, fit.measures = T)
...
```{r mod4 fitting}
RICLPM_mod4.fit <- lavaan(RICLPM_mod1_ext,
  data = dat_SDQ_nonASD, 
  missing = "ML", 
  meanstructure = T, 
  int.ov.free = T
