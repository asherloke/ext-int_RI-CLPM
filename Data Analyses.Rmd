...
dat_SDQ <- dat_SDQ %>% mutate(ethnicity = recode(ethnicity, 
                                     "Bangladeshi" = "non-White", 
                                     "Black African" = "non-White", 
                                     "Black Caribbean" = "non-White", 
                                     "Chinese" = "non-White", 
                                     "Indian" = "non-White", 
                                     "Mixed" = "non-White", 
                                     "Other Black" = "non-White",
...
```{r Wave 2}
w2_sdq <- bind_rows(desc_stat(dat_SDQ, Diff2),
  ...
  mutate(wave = 2,
         subscale = recode(subscale, 
                           "Diff2" = "Diff",
                           "Prosocial2" = "Prosocial",
                           "Peer2" = "Peer",
                           "Hyper2" = "Hyper",
                           "Conduct2" = "Conduct",
                           "Emotional2" = "Emotional"))
w2_sdq
...
```{r Wave 3}
w3_sdq <- bind_rows(desc_stat(dat_SDQ, Diff3),
  ...
  mutate(wave = 3,
         subscale = recode(subscale, 
                           "Diff3" = "Diff",
                           "Prosocial3" = "Prosocial",
                           "Peer3" = "Peer",
                           "Hyper3" = "Hyper",
                           "Conduct3" = "Conduct",
                           "Emotional3" = "Emotional"))
w3_sdq
...
```{r Wave 4}
w4_sdq <- bind_rows(desc_stat(dat_SDQ, Diff4),
  ...
  mutate(wave = 4,
         subscale = recode(subscale, 
                           "Diff4" = "Diff",
                           "Prosocial4" = "Prosocial",
                           "Peer4" = "Peer",
                           "Hyper4" = "Hyper",
                           "Conduct4" = "Conduct",
                           "Emotional4" = "Emotional"))
w4_sdq
...
```{r Wave 5}
w5_sdq <- bind_rows(desc_stat(dat_SDQ, Diff5),
  ...
  mutate(wave = 5,
         subscale = recode(subscale, 
                           "Diff5" = "Diff",
                           "Prosocial5" = "Prosocial",
                           "Peer5" = "Peer",
                           "Hyper5" = "Hyper",
                           "Conduct5" = "Conduct",
                           "Emotional5" = "Emotional"))
w5_sdq
...
```{r Wave 6}
w6_sdq <- bind_rows(desc_stat(dat_SDQ, Diff6),
  ...
  mutate(wave = 6,
         subscale = recode(subscale, 
                           "Diff6" = "Diff",
                           "Prosocial6" = "Prosocial",
                           "Peer6" = "Peer",
                           "Hyper6" = "Hyper",
                           "Conduct6" = "Conduct",
                           "Emotional6" = "Emotional"))
w6_sdq
...
```{r Wave 7}
w7_sdq <- bind_rows(desc_stat(dat_SDQ, Diff7),
  ...
  mutate(wave = 7,
         subscale = recode(subscale, 
                           "Diff7" = "Diff",
                           "Prosocial7" = "Prosocial",
                           "Peer7" = "Peer",
                           "Hyper7" = "Hyper",
                           "Conduct7" = "Conduct",
                           "Emotional7" = "Emotional"))
w7_sdq
...
## Grouping differences (ethnicity vs ASD)

```{r}
results_chi_eth <- chisq.test(x = dat_SDQ %>% pull(ethnicity), # 1st grouping variable
                      y = dat_SDQ %>% pull(ASD_final),
                      correct = T) %>% # 2nd grouping variable
  tidy()

results_chi_eth
#chi(1) = 8.70, p=.003

Phi_eth <- cramersV(x = dat_SDQ %>% pull(ethnicity), 
                     y = dat_SDQ %>% pull(ASD_final),
                    correct = T)
Phi_eth
#Phi φ = .02
...
```{r plot}
bind_rows(w2_sdq, w3_sdq, w4_sdq, w5_sdq, w6_sdq, w7_sdq) %>% 
  ggplot(aes(x = wave, y = mean, colour = ASD_final)) +
  geom_point() + 
  geom_smooth(method = lm, se = T) +
  facet_wrap(~subscale) +
  theme_bw()
ggsave("subscale_plot.jpg")
```
...
'
RICLPM1_mod1.fit <- sem(RICLPM_mod1, 
  data = dat_SDQ, 
  missing = 'ML'
)
summary(RICLPM1_mod1.fit, standardized = T, fit.measures = T)
...
RICLPM_mod2 <- '
  ...
  # autoregressive and cross-lagged effects across groups)
  ...
  RI_Conduct + RI_Hyper + RI_Emotional + RI_Peer ~~ 0*Int2_F + 0*Ext2_F
'
RICLPM1_mod2.fit <- sem(RICLPM_mod2, 
  data = dat_SDQ, 
  missing = 'ML',
  group = "ASD_final"
)
summary(RICLPM1_mod2.fit, standardized = T, fit.measures = T)
lavInspect(RICLPM1_mod2.fit, "cov.lv")
...
```{r Comparisons}
comp_fit <- compareFit(RICLPM1_mod1.fit, RICLPM1_mod2.fit) # constrained lagged coefficients relevant to study
...
```{r mod3 fitting}
RICLPM_mod3.fit <- sem(RICLPM_mod1,
  data = dat_SDQ_ASD, 
  missing = "ML"
)
summary(RICLPM_mod3.fit, standardized = T, fit.measures = T)
lavInspect(RICLPM_mod3.fit, "cov.lv")
...
```{r mod4 fitting}
RICLPM_mod4.fit <- sem(RICLPM_mod1,
  data = dat_SDQ_nonASD, 
  missing = "ML"
)
...
# Sensitivity Analysis: RI-CLPM with conduct and emotional subscales
...
```{r mod5: basic model for both ASD and non-ASD}
RICLPM_mod5 <- '
  # Create between components (random intercepts)
  CM_Emotional =~ 1*Emotional2 + 1*Emotional3 + 1*Emotional4 + 1*Emotional5 + 1*Emotional6 + 1*Emotional7
  CM_Conduct =~ 1*Conduct2 + 1*Conduct3 + 1*Conduct4 + 1*Conduct5 + 1*Conduct6 + 1*Conduct7
  
  # Create within-person centered variables
  L_Emotional2 =~ 1*Emotional2
  L_Emotional3 =~ 1*Emotional3
  L_Emotional4 =~ 1*Emotional4
  L_Emotional5 =~ 1*Emotional5
  L_Emotional6 =~ 1*Emotional6
  L_Emotional7 =~ 1*Emotional7
  L_Conduct2 =~ 1*Conduct2
  L_Conduct3 =~ 1*Conduct3
  L_Conduct4 =~ 1*Conduct4
  L_Conduct5 =~ 1*Conduct5
  L_Conduct6 =~ 1*Conduct6
  L_Conduct7 =~ 1*Conduct7

  # Estimate lagged effects between within-person centered variables
  L_Conduct3 + L_Emotional3 ~ L_Conduct2 + L_Emotional2
  L_Conduct4 + L_Emotional4 ~ L_Conduct3 + L_Emotional3
  L_Conduct5 + L_Emotional5 ~ L_Conduct4 + L_Emotional4
  L_Conduct6 + L_Emotional6 ~ L_Conduct5 + L_Emotional5
  L_Conduct7 + L_Emotional7 ~ L_Conduct6 + L_Emotional6

  # Estimate covariance between within-person centered variables at first wave
  L_Conduct2 ~~ L_Emotional2 # Covariance
  
  # Estimate covariances between residuals of within-person centered variables 
  # (i.e., innovations)
  L_Conduct3 ~~ L_Emotional3
  L_Conduct4 ~~ L_Emotional4
  L_Conduct5 ~~ L_Emotional5
  L_Conduct6 ~~ L_Emotional6
  L_Conduct7 ~~ L_Emotional7
  
  # Estimate variance and covariance of random intercepts
  CM_Emotional ~~ CM_Emotional
  CM_Conduct ~~ CM_Conduct
  CM_Emotional ~~ CM_Conduct

  # Estimate (residual) variance of within-person centered variables
  L_Conduct2 ~~ L_Conduct2 # Variances
  L_Emotional2 ~~ L_Emotional2 
  L_Conduct3 ~~ L_Conduct3 # Residual variances
  L_Emotional3 ~~ L_Emotional3 
  L_Conduct4 ~~ L_Conduct4 
  L_Emotional4 ~~ L_Emotional4 
  L_Conduct5 ~~ L_Conduct5 
  L_Emotional5 ~~ L_Emotional5 
  L_Conduct6 ~~ L_Conduct6
  L_Emotional6 ~~ L_Emotional6
  L_Conduct7 ~~ L_Conduct7
  L_Emotional7 ~~ L_Emotional7
'
RICLPM_mod5.fit <- lavaan(RICLPM_mod5,
  data = dat_SDQ, 
  missing = "ML", 
  meanstructure = T, 
  int.ov.free = T
)
summary(RICLPM_mod5.fit, standardized = T, fit.measures = T)
#cfi = .980, RMSEA = .041, SRMR = .041
```
...
RICLPM_mod6 <- '
  ...
  L_Conduct3 ~ c(a1, a1)*L_Conduct2 + c(b1, b1)*L_Emotional2
  L_Emotional3 ~ c(c1, c1)*L_Conduct2 + c(d1, d1)*L_Emotional2
  L_Conduct4 ~ c(a2, a2)*L_Conduct3 + c(b2, b2)*L_Emotional3
  L_Emotional4 ~ c(c2, c2)*L_Conduct3 + c(d2, d2)*L_Emotional3
  L_Conduct5 ~ c(a3, a3)*L_Conduct4 + c(b3, b3)*L_Emotional4
  L_Emotional5 ~ c(c3, c3)*L_Conduct4 + c(d3, d3)*L_Emotional4 
  L_Conduct6 ~ c(a4, a4)*L_Conduct5 + c(b4, b4)*L_Emotional5
  L_Emotional6 ~ c(c4, c4)*L_Conduct5 + c(d4, d4)*L_Emotional5
  L_Conduct7 ~ c(a5, a5)*L_Conduct6 + c(b5, b5)*L_Emotional6
  L_Emotional7 ~ c(c5, c5)*L_Conduct6 + c(d5, d5)*L_Emotional6
  ...
  L_Emotional7 ~~ L_Emotional7
'
RICLPM_mod6.fit <- lavaan(RICLPM_mod6,
  data = dat_SDQ, 
  missing = "ML", 
  group = "ASD_final",
  meanstructure = T, 
...
summary(RICLPM_mod6.fit, standardized = T, fit.measures = T)
#cfi = 0.979, RMSEA = .036, SRMR = .040
```

## Comparisons

```{r Comparisons}
comp_fit <- compareFit(RICLPM_mod5.fit, RICLPM_mod6.fit)
summary(comp_fit,  fit.measures = c("cfi", "rmsea", "srmr"))
```

## mod7: version of mod 1 for ASD group only
...
```{r mod7 fitting}
RICLPM_mod7.fit <- lavaan(RICLPM_mod5,
  data = dat_SDQ_ASD, 
  missing = "ML", 
  meanstructure = T, 
  int.ov.free = T
)
summary(RICLPM_mod7.fit, standardized = T, fit.measures = T)
#cfi = .990, RMSEA = .037, SRMR = .029
```

## mod8: version of mod 1 for non-ASD group only
...
```{r mod8 fitting}
RICLPM_mod8.fit <- lavaan(RICLPM_mod5,
  data = dat_SDQ_nonASD, 
  missing = "ML", 
  meanstructure = T, 
  int.ov.free = T
)
summary(RICLPM_mod8.fit, standardized = T, fit.measures = T)
#cfi = .988, RMSEA = .036, SRMR = .035
```

# Further exploratory work not reported (Sensitivity Analysis: RI-CLPM with tabulated exter-inter scores)
